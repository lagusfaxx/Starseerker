 ...
 * El `code` es el identificador que se guarda en pedidos y zonas de envío.
 */
export type Region = {
  code: string;
  name: string;
...
export function comunasOf(code: string | null | undefined): string[] {
  if (!code) return [];
  return REGION_BY_CODE.get(code)?.comunas ?? [];
}
