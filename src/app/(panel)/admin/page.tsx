...
import { getContentChecklist } from '@/lib/content-checklist';
...
export default async function AdminDashboardPage() {
  ...
  const [stats, checklist, recentOrders] = await Promise.all([
    ...
    <>
      ...
      {!checklist.ready ? (
        <section className="mt-8 border-l-4 border-amber-500 bg-amber-50 p-6">
          ...
          <p className="mt-2 max-w-3xl text-sm text-amber-900/80">
            El catalogo se cargo con lo minimo verificable: nombre, categoria y SKU. Los precios,
            las descripciones y el stock son informacion del negocio y deben cargarse con los datos
            oficiales antes de vender.
          </p>
          <ul className="mt-4 space-y-2 text-sm text-amber-900">
            {checklist.productsWithoutStock > 0 ? (
              <ChecklistItem href="/admin/productos">
                {checklist.productsWithoutStock} de {checklist.totalProducts} productos sin stock
                cargado. Mientras siga en cero no se pueden comprar.
              </ChecklistItem>
            ) : null}
            {checklist.productsWithoutDescription > 0 ? (
              <ChecklistItem href="/admin/productos">
                {checklist.productsWithoutDescription} productos sin descripcion.
              </ChecklistItem>
            ) : null}
            {checklist.productsWithoutFeatures > 0 ? (
              <ChecklistItem href="/admin/productos">
                {checklist.productsWithoutFeatures} productos sin caracteristicas ni
                especificaciones.
              </ChecklistItem>
            ) : null}
            {!checklist.hasLogo ? (
              <ChecklistItem href="/admin/ajustes">
                Falta subir el logo de la tienda. Mientras tanto se muestra el nombre en texto.
              </ChecklistItem>
            ) : null}
          </ul>
          <p className="mt-4 text-xs text-amber-900/70">
            Revisa tambien los precios: los del catalogo inicial son valores de referencia, no una
      ...
      {stats.ordersToShip > 0 ? (
        <Link
          ...
          className="mt-6 flex items-center justify-between gap-4 border-l-4 border-brand bg-black px-6 py-4 transition-colors hover:bg-brand-50"
        >
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[2fr_1fr]">
        <section className="border border-sand-dark bg-black p-6">
...
function ChecklistItem({ href, children }: { href: string; children: React.ReactNode }) {
  return (
...
function StatCard({
  ...
  return (
    <div className="border border-sand-dark bg-black p-6">
