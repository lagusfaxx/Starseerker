...
export default async function AdminStripsPage() {
  ...
  return (
    <>
      ...
      {strips.length === 0 ? (
        <p className="mt-8 border border-dashed border-sand-dark bg-black px-6 py-14 text-center text-sm text-ink-muted">
        ...
        <ul className="mt-8 space-y-4">
          {strips.map((strip) => (
            <li key={strip.id} className="border border-sand-dark bg-black">
              <Link
