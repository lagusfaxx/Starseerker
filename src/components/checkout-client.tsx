            ...
            {transfer ? (
              ...
              >
                {method === 'transferencia' ? (
                  ...
                  <div className="mt-4 border border-sand-dark bg-black px-4 py-4 sm:px-5">
            ...
            {lines.map((line) => (
              <li key={line.key} className="flex gap-3">
                <div className="relative h-16 w-16 shrink-0 bg-black">
                  ...
                  <span className="absolute -right-2 -top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-ink px-1 text-[10px] font-bold text-black">
                    {line.quantity}
                  </span>
                </div>
  ...
  return (
    <button
      ...
      className={`flex h-full items-start gap-3 border-2 p-5 text-left transition-colors ${
        seleccionado ? 'border-ink bg-sand' : 'border-sand-dark bg-black hover:border-ink-soft'
      }`}
    >
