...
Tienda online del distribuidor oficial de **STARSEEKER** en Chile: catalogo,
carrito, checkout con **Mercado Pago**, seguimiento de envios, cuentas de
cliente y panel de administracion. Pensada para desplegarse en **Coolify** con
Docker, en el dominio `starseerker.cl`.
...
npm run db:seed     # crea colecciones, cupones y el administrador
...
| `npm run db:seed` | Crea colecciones, cupones y administrador (productos solo con `SEED_CATALOG=true`) |
...
| Variable | Por defecto | Descripcion |
| --- | --- | --- |
| `STORE_NAME` | `STARSEEKER Chile` | Nombre mostrado en la tienda |
...
| Imagenes | Sin cargar | La fotografia oficial de STARSEEKER la subes tu desde el panel; el repositorio no la redistribuye |
...
4. Pon la clave en `RESEND_API_KEY` y una direccion de ese dominio en
   `EMAIL_FROM` (por ejemplo `STARSEEKER Chile <pedidos@starseerker.cl>`).
...
Medido en esta misma tienda: antes la palabra "STARSEEKER" aparecia **cero
veces** en el HTML de la portada; con el campo cargado, 48.
...
3. En **Environment Variables** pega el contenido de `.env.example` con tus
   ...
   APP_URL=https://starseerker.cl
   SESSION_SECRET=<openssl rand -base64 48>
   MP_ACCESS_TOKEN=APP_USR-...
   MP_WEBHOOK_SECRET=...
   MP_CURRENCY=CLP
   ADMIN_EMAIL=admin@starseerker.cl
   ADMIN_PASSWORD=una-clave-fuerte
   ```

   De Mercado Pago solo hacen falta esas dos: el **Access Token** y el
   **secreto del webhook**. La Public Key, el Client ID y el Client Secret son
   de otros modos de integracion y esta tienda no los lee.

4. En **Domains** asigna `https://starseerker.cl` al servicio `app`, puerto
   `3000`. El compose usa `expose: 3000` y no `ports`, para que el proxy de
   Coolify enrute por dominio sin ocupar el puerto del servidor.
...
3. Cargar los datos iniciales **solo si la base esta vacia** (colecciones,
   cupones y el usuario del panel; productos unicamente si defines
   `SEED_CATALOG=true`).
4. Arrancar el servidor.

Para desactivar el seed automatico define `SKIP_SEED=true`.

> "Base vacia" se decide mirando si existe algun usuario, no algun producto: el
> catalogo puede quedar legitimamente en cero porque los productos solo se
> siembran con `SEED_CATALOG=true`.

**`SEED_CATALOG`** (por defecto `false`). En `true` el seed agrega los 16
equipos del catalogo oficial de STARSEEKER con lo unico verificable —nombre,
SKU y coleccion— como **borradores**, con precio 0, stock 0 y sin publicar.
Sirve para no teclear el catalogo a mano; los precios, las descripciones, las
especificaciones y las fotos los cargas tu antes de activarlos.
...
el nombre si lo dejas vacio. Las fotos se suben desde tu equipo en el mismo
formulario (o pegas una URL); la primera es la principal y el orden se cambia
con las flechas.
...
`prisma/seed.ts` y ejecuta `SEED_CATALOG=true npm run db:seed`. El seed es
idempotente y nunca pisa lo que ya editaste en el panel.
...
El repositorio no incluye ni una sola imagen de producto: la fotografia oficial
de STARSEEKER es del fabricante y no se redistribuye aqui. Las fotos se suben
desde el panel, se guardan en PostgreSQL y se sirven desde `/api/media/<id>`,
asi que sobreviven a los redespliegues de Coolify sin montar ningun volumen.
...
icono del navegador (`public/icon.svg`) es una figura geometrica neutra que
puedes reemplazar por el tuyo desde el mismo formulario.

La tienda es monocroma a proposito —negro, blanco y grises— para que el color
lo pongan las fotos del producto y no la interfaz.
